...
class AgentMemory(Base, WorkspaceOwnedMixin):
    ...
    derived_from_episode_ids: Mapped[list[str]] = mapped_column(JSON, nullable=False, default=list)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True),
        nullable=False,
        server_default="now()",
...
class Lesson(Base, WorkspaceOwnedMixin):
    ...
    confidence: Mapped[Decimal | None] = mapped_column(Numeric(6, 4), nullable=True)
    supporting_episode_ids: Mapped[list[str]] = mapped_column(JSON, nullable=False, default=list)
