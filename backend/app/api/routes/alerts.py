...
class AlertTriggerBody(BaseModel):
    price: float = Field(description="Mock market price for integration tests")
...
async def trigger_alert_mock(
    ...
    if not fired:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Alert condition not met")
