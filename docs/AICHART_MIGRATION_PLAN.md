...
| D5 | **الإشعارات الخارجية التي يبدأها النظام — خارج النطاق** | يبقى بريد التحقق/استعادة كلمة المرور فقط. **مُعدَّل بـ D13** |
...
| **D13** | **تلجرام قناة محادثة** — المستخدم يربط حسابه ويتحدث مع **الوكيل نفسه**، بلا رسالة واحدة يبدأها النظام | ‏ADR 0009؛ يعدّل D5 جزئياً |
...
**والمكتبة ضُمِّنت.** الأصول الزمنية (~26 ميجابايت في ~1900 قطعة) عبر **Git LFS**، وتعريفات الأنواع
(~1.2 ميجابايت نصّاً) في git عادي — لأن TypeScript يقرؤها في كل `tsc`، ومؤشّر LFS غير مُفكوك يكسر
الترجمة حيث لم يعمل المرشِّح. وحُذف `klinecharts` من التبعيات ومن القفل.

**الترخيص يفرضه البناء:** `scripts/check_repo_visibility.sh` يفشل إن أعلن المستودع أنه عام والمكتبة
موجودة. «أبقِ المستودع خاصاً» إعدادٌ يستطيع أحدهم قلبه في واجهة ويب بعد أشهر، بلا diff وبلا مراجعة —
ولحظة قلبه تصير 27 ميجابايت من كود مرخَّص إعادةَ توزيع علنية.

**والحدّ صمد:** استبدال العارض مسّ ملفَّين فقط، لأن كل مستهلك يرتبط بـ`ChartEngine` و
`AnnotationSurface`. و`rendererBoundary.test.ts` يبقيه كذلك — أول مكوّن يستورد الودجت مباشرةً سيعمل
تماماً، وسيكلّف التجريد بصمت.

**وأصلح النقل عيبَين في طريقه:**
- `ChartAnnotationRenderer` كان يربط الأنواع الدلالية بأسماء طبقات KLineChart (`DRAW_ZONE` → `"rect"`)
  **داخل الصنف الوحيد المفترض أن يكون مستقلاً عن العارض**. اختيار الأداة صار مهمّة السطح، لأنه وحده
  يعرف ما لديه من أدوات.
- منتقي الإطار الزمني كان ما زال يعرض **M30 وD1** بعد أن صارت المنصة سكالب-فقط (D11) — أي أن الواجهة
  تسرد إطارَين ترفضهما الـAPI بـ422. وخيارٌ لا يمكن أن يعمل أسوأ من غياب الخيار: الفشل يُقرأ كخلل في
  التحليل لا كإطار غير موجود.

> **وملاحظة على حارسي نفسه:** أول نسخة من `rendererBoundary.test.ts` فشلت في التجميع (`scandir '/src'`)
> — ومع ذلك أبلغ التشغيل الكامل عن «94 ناجحاً». حارس لا يعمل ويُبلِّغ بالنجاح هو الشيء الوحيد الذي
> يجب ألّا يفعله حارس. أُصلح المسار، وأُضيف تأكيد أن الحارس **قرأ الشجرة فعلاً**.
...
## 11. سجل الإنجاز النهائي

**كل المراحل M0–M11 مكتملة.** هذا الجدول هو الحالة الختامية؛ التفاصيل في
[`PARITY_MATRIX.md`](PARITY_MATRIX.md) ورسائل الالتزامات على الفرع.

| المرحلة | الحالة | الشاهد |
|---------|--------|--------|
| M0 الحواجز والعقود | ✅ (القسم أعلاه) | ثغرة `watchlist_items` أُصلحت؛ حرّاس المطابقة تعمل منذ اليوم الأول |
| M1 بيانات السوق | ✅ | الذهب فقط (D10)؛ `require_instrument` يرمي ولا يستبدل |
| M2 البدائيات | ✅ | تفاضل ذهبي كامل عند `1e-9`؛ `js_round`/`js_to_fixed` في `core/numeric.py` |
| M3 محرك الهندسة | ✅ | عقد الحدود منقول حرفياً؛ `confirmed` كان غير قابل للبلوغ في المرجع وأُصلح |
| M4 المحرّكات التحليلية | ✅ | صفر عناصر نائبة؛ فرعا `plan_sanity` و`volatile` الميّتان أُحييا بالقياس |
| M5 بنية LLM | ✅ | سجل مطالبات مُجزَّأ + `prompt_hash`؛ تضمينات حقيقية؛ حلقة أدوات مسقوفة |
| M6 خط أنابيب الوكيل | ✅ | رسم المراحل محفوظ؛ `plan_sanity` بوابة حتمية مكان `execution_guard` |
| M7 دورة حياة التوصية | ✅ | لا دالة تقبل مُعامل نطاق؛ إعادة التقييم مسقوفة وبمنع تكرار على مستوى القاعدة |
| M8 حلقة التعلّم | ✅ | سلامة النتائج بمفتاح فريد؛ الحالات عالمية والإحصاءات معزولة ولا تُجمعان |
| M9 طبقة الشارت | ✅ كود | 24 نوعاً دلالياً + 18 دوراً؛ الثنائيات محجوبة بميزانية LFS (بند مالك) |
| M10 العربية والتصميم | ✅ | 252 مفتاحاً بتكافؤ مُختبَر؛ حرّاس الخصائص المنطقية والتباين AA بالقياس الفعلي |
| M11 التكافؤ والمراقبة | ✅ | 34 أداة في سجل واحد بناقلَين؛ Prometheus حقيقي؛ مصفوفة التكافؤ موقَّعة |

**الأرقام الختامية:** 1221 اختبار خلفية + 106 اختبارات واجهة، ‏mypy strict على 383 ملفاً، ‏ruff نظيف.

**بنود المالك المتبقية** (ليست كوداً — [`BLOCKED_ON_OWNER.md`](BLOCKED_ON_OWNER.md)):
ميزانية Git LFS لثنائيات TradingView · إصلاح مزوّد GitHub Actions ‏(`runner_id: 0` على كل تشغيل
في تاريخ المستودع) · وسم مرجع AiChart وأرشفته.
