...
إطلاقاً: وقف يقع داخل السبريد الحيّ، أو هدف لا يغطّي رحلة الذهاب والإياب، هو **توصية سيئة** على منصة
لا تنفّذ شيئاً. أُسقطت آلية الأوامر كلها وبقي الحساب: كلفة الجولة الكاملة (سبريد وانزلاق يُدفعان
مرتين)، وحاجز `MIN_COST_MULTIPLE = 3`، وسياسة مخاطرة لكل إطار قرار، ومضاعفات الجلسات، و**قاعدة
المصدر**: عند غياب سبريد مرصود يُعاد النموذج **موسوماً كنموذج** (`static_model`) لا كرقم يوحي بأنه
مقيس.
